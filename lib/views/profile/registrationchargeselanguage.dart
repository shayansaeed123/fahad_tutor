...
class RegistrationChargesELanguage extends StatefulWidget {
  const RegistrationChargesELanguage({super.key});

  @override
  State<RegistrationChargesELanguage> createState() => _RegistrationChargesELanguageState();
}

class _RegistrationChargesELanguageState extends State<RegistrationChargesELanguage> {
...
Future<void> _uploadImages() async {
    ...
    request.fields['Registration_english_check'] = '1';
        if (_chargesSlip != null) {
          request.files.add(await http.MultipartFile.fromPath('Registration_english', _chargesSlip!.path));
        }
    ...
    if (response.statusCode == 200) {
      ...
      setState(() {
        ...
        repository.charges_image_english.value = responseData['Registration_english'] ?? repository.charges_image_english.value;
      });
  ...
  void _validateForm() {
  if (repository.charges_image_english.value != 'https://www.fahadtutors.com/fta_admin/' &&
      repository.charges_image_english.value.isNotEmpty ) {
    _uploadData();
  } else {
    Utils.snakbar(
      context,
      repository.charges_image_english.value == 'https://www.fahadtutors.com/fta_admin/' || repository.charges_image_english.value.isEmpty
  ...
  Future<void> _uploadData() async {
    ...
    try {
      final response = await http.post(
        ...
        'payment_recipt_english_l': repository.charges_image_english.value.toString(),
      },);
      if (response.statusCode == 200) {
  ...
  Widget build(BuildContext context) {
    return reusableprofileidget( context,
                  ...
                  child: Stack(
                    children: [
                      Column(
                        ...
                        children: [
                          reusableText(repository.registration_heading_english,color: colorController.blackColor,fontsize: 23,fontweight: FontWeight.bold),
            ...
            reusablaSizaBox(context, 0.020),
                          reusableText(repository.Registration_text_english,color: colorController.blackColor,fontsize: 14),
          ...
          reusableDocuments1(context, '', '', repository.charges_image_english.value.toString(), (){
              reuablebottomsheet(context, 'Choose Charges Slip Image', (){
                _pickImage(ImageSource.gallery);
              }, (){
                _pickImage(ImageSource.camera);
